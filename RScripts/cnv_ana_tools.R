...
assess_significance <- function(ratio_file, cnvs_file) {
  ...
  # changes 10.05.2024: Adding some tryCatches
  tryCatch({
	...
	cnvs <- data.frame(dataTable)
	numberOfCol <- length(cnvs)
	cnvs[,numberOfCol + 1] <- NA
	cnvs[,numberOfCol + 2] <- NA
	names(cnvs) <- c(
	      "chr",
	      "start",
	      "end",
	...
	cat("\nBeim Berechnen des assess_significance ist ein Fehler aufgetreten.\n")
	return(cnvs)		
  })
}
...
# Try annotation first with SQL server if this fails try with biomaRt, if no internet connection available use offline
  if (cnv_region_annotation == "online") {
    ...
    for(i in 1:nrow(x)) {
      ...
      if (!is.na(x$WilcoxonRankSumTestPvalue[i])
          & !is.na(x$KolmogorovSmirnovPvalue[i])     
          & x$WilcoxonRankSumTestPvalue[i] < 0.05
        ...
        if (inherits(query, 'try-error')) {
          ...
          if (inherits(query, 'try-error')) {
            warning("Check your internet connection. Connection to USCS SQL and biomaRt \
            server failed. Using offline annotation!")
...
cnv_processing <- function(
  ...
  if(ncol(cnv) == 0) {
    print("Keine CNVs detektiert.")
    return(list(loss_go = NULL, gain_go = NULL))
  }
  else {
    cnv.genes <- c()
    ...
    tryCatch({
      ...
      return(list(loss_go = loss_go, gain_go = gain_go))
        ...
        cat("\nBei cnv_processing ist ein Fehler aufgetreten.\n")
        return(list(loss_go = NULL, gain_go = NULL))
      })
  }
}
...
cnv_table <- function(cnvs, ampl_genes) {
  ...
  if(length(cnvs$genes) == 0) {
    print("Keine CNVs detektiert.")
    return(data.frame(Gene = NULL, Chr = NULL, Start = NULL, End = NULL, CopyNumber = NULL, Status = NULL))
  }
