...
kable(cnvs, caption = "Identifizierte \\acp{CNV}\\label{cnv_og}", booktabs = T, row.names = F, digits = 2, longtable = TRUE) %>% kable_styling(latex_options = c("striped", "HOLD_position"), font_size = 9, full_width = F) %>% column_spec(c(1), width = "5em") %>% column_spec(c(2:3), width = "1em") %>% row_spec(0, angle = 60)
@
...
if(!is.null(tmp)){
  ...
  if(length(id) > 0) {
    ...
    tmp$plots <- "Kein Plot verf\"ugbar."
  ...
  tmp <- tmp[, c(1:2, 5)]
  colnames(tmp)[3] <- "Ansicht des Fusionsproduktes mit Angabe der Domainen"
  kable(tmp, format = "latex", escape = FALSE, caption = "Plot der identifizierten Fusionen. Der senkrechte Strich markiert den Bruchpunkt. \\label{fs_plot}", booktabs = T, longtable = T, digits = 3, row.names = F) %>% kable_styling(latex_options = c("striped", "HOLD_position", "repeat_header"), font_size = 10) %>% column_spec(1,extra_css = "vertical-align:middle;")
  }
@
