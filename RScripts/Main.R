...
if (protocol == "panelTumor") {
  tumor <- paste0(
    path_input, strsplit(x = tumor, split = ".", fixed = T)[[1]][1],
    "_R1_fastqc/Images/per_base_quality.png"
  )
  tumor_bsqr <- paste0(
    path_input, sample,
...
if (file.exists(fus_file)) {
  ...
  if (!is.null(fusions$Table)) {
    if (dim(fusions$Table)[1] != 0) {
      ...
      if (any(is.na(fusions$Plots$file)) || any(is.null(fusions$Plots$file))) {
        fus_38_files <- paste(
          fusions$Plots$paths[!is.na(fusions$Plots$paths)],
          fusions$Plots$file[!is.na(fusions$Plots$file)], sep = "/"
        )
        for (i in 1:length(fus_38_files)) {
          system(paste0("cp ", fus_38_files[i], " ", path_output, "/", fus_38_files[i], "_", i))
        }
      } else {
        fus_38_files <- paste(
          fusions$Plots$paths,
